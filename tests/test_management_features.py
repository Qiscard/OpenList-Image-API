...
class WebUiMarkupTests(unittest.TestCase):
    def test_gallery_uses_slideshow_and_stable_waterfall(self) -> None:
        page = gallery_html()
        self.assertIn("requestImages(WATERFALL_BATCH_SIZE,'waterfall')", page)
        self.assertIn("WATERFALL_BATCH_SIZE=20", page)
        self.assertIn("SLIDE_PRELOAD_COUNT=2", page)
        self.assertIn("URL_RESOLVE_CONCURRENCY=6", page)
        ...
        self.assertIn("id=\"waterfall-sentinel\"", page)
        self.assertIn("setWaterfallBufferVisible", page)
        self.assertIn("usesSharedImageChain()", page)
        self.assertIn("applySharedChain(data,kind)", page)
        self.assertIn("&chain=", page)
        self.assertIn("&offset=", page)
        self.assertIn("CHAIN_PROGRESS_KEY", page)
        self.assertIn("GALLERY_RESTORE_KEY", page)
        self.assertIn("ADMIN_RETURN_KEY", page)
        self.assertIn("retargetLoadedImages()", page)
        self.assertIn("unseenImages(batch,chainId)", page)
        self.assertIn("render({keepChain:true})", page)
        self.assertIn("render({keepChain:true,restore:consumeAdminReturn()})", page)
        self.assertIn("data-admin-link", page)
        self.assertIn("markAdminRestore()", page)
        self.assertIn("if(previous.preview_quality!==settings.preview_quality||previous.lightbox_quality!==settings.lightbox_quality) retargetLoadedImages();", page)
        self.assertNotIn("sharedChainOrigin()", page)
        self.assertNotIn("SHARED_CHAIN_WINDOW_MS", page)
        self.assertIn("@keyframes card-shimmer", page)
        self.assertIn(".card.is-loading .preview-button::after", page)
        ...
        self.assertIn("const URL_RESOLVE_CONCURRENCY=6", page)
        self.assertIn("function retargetCard(image)", page)
        self.assertIn("attempts>=2", page)
...
class InstallerTests(unittest.TestCase):
    def test_embedded_installer_uses_fixed_proxy_candidates(self) -> None:
        ...
        self.assertIn('"url_cache_size": 4000', installer)
        self.assertIn('"url_cache_ttl_seconds": 7200', installer)
