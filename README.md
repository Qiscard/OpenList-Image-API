...
- **瀑布流浏览**：每批加载 20 张，宽屏（>900 px）3 列、中等宽度（561–900 px）2 列；移动端（≤560 px）可在设置中选择单列或双列。新卡进入当前最矮列，占位时显示加载动画，接近可视区时才换链，并在滚动到页面 80% 后继续取下一批；预取下一批时底部显示缓冲动画。批量换链在后台进行，不挡住卡片入列。
- **共享随机链**：未筛选浏览时，所有访客共用当前约 4000 张的热点链，各自按 `chain`/`offset` 分页；链被看完或每 2 小时整点窗口切换到下一条，上一链在过期前仍可继续翻完。当前浏览器会记住已加载图片，刷新/重载跳过这些图；从管理页返回会恢复当前画面。仅改画质时不整页刷新。带标签/目录/大小筛选时仍各自独立随机。
- **批量换链**：浏览页先获取随机图片元数据并立刻入列，再通过 `POST /api/download-url` 的 `preview:true` 在后台批量解析缩略图；已完成的地址会回填到卡片上。批量失败或超时后，视口内图片再按最多 6 路并发单图请求补拉，原图签名 URL 只在灯箱或下载时解析，相同路径的并发请求会合并。
...
- **公告与维护模式**：支持 Markdown 公告（含 `![说明](https://...)` 公网图片）、强制阅读倒计时和按公告版本记忆关闭状态；可单独启用联系按钮，支持个人和群组两条联系方式，每条可配图片（二维码等）和跳转链接。点击按钮展开/关闭弹层，一行最多两格、宽度按条目数自适应，图片在上文字在下，无图片时只显示文字，点击图片或文字跳转对应链接。维护期间浏览相关 API 返回 503，管理员可在当前会话中使用管理令牌临时解锁。索引重建只在管理页启动，并显示无数据、日期数据、重建中或重建完毕状态。
- **实时目录浏览**：管理页按需读取 OpenList 目录树，不维护单独的目录缓存；图片索引仍需显式重建。一刻相册（OpenList `BaiduPhoto`）只作为可勾选的存储器根，不展开数百个作者子目录；重建索引时自动只收录纯数字作者目录中的图片。
- **索引与浏览性能**：重建是后台一次性任务（约 300 个作者相册时，本机 OpenList 通常约 1.5–5 分钟；对接百度一刻上游的生产机可能需要更久，根目录列举单页就常超过 10 秒），浏览随机图/瀑布流只读本地 `index.json` 再按需换链，不因作者目录数量在每次请求时重扫存储。索引列表超时 60 秒、每页 100 条；换链对 OpenList `fs/get` 等待 45 秒、批量接口最多等 25 秒（一刻单图常需十几到几十秒），并发换链 worker 为 6，避免过短超时把上游请求 cancel 成 502/空白图。
...
- 目录显示开关和隐藏层数；
- 浏览页默认主题；
- 公告、QQ 联系按钮、维护模式、标签策略和日志级别。

管理页主题独立保存在浏览器中，默认暗色。多名管理员并发保存配置时，以最后一次成功保存为准。

公告的“本次关闭”实际在当前浏览器中隐藏到当天结束；“不再显示”会永久隐藏当前公告版本。公告内容或相关设置改变时版本递增，新版本会重新显示。公告图片使用 Markdown `![说明](https://example.com/notice.png)`，地址必须是可公网访问的 `http://` 或 `https://`；OpenList 签名链接过期后会失效，不适合作为公告图。
...
`preview: true` 只解析缩略图，响应中的 `url` 保持为空；省略该字段时解析原图签名 URL。服务端最多处理 50 个有效去重路径，单批等待上限为 4 秒，超时、未索引和上游失败会在对应项返回 `error`。超时后上游解析仍继续填缓存，不取消。`fresh=true` 会刷新已缓存的 URL 或缩略图。浏览器批量预览请求失败时，会退回最多 6 路并发的单图请求。

大图中的“下载”按钮会刷新并点击 OpenList 签名 URL；独立的 `/download` 路由用于需要服务端附件代理语义的 API 调用方。

## 缓存与并发

- 服务共享 12 个 URL 解析线程；浏览器批量预览失败后的单图补偿队列最多并发 6 路。
- 相同路径、相同模式的并发未命中会合并为一次 OpenList 请求，即使缓存容量为 0 也生效。
- 源码缺省配置的缓存容量为 0；安装器生成的部署配置使用 4000 条、7200 秒。
...
- 旧安装配置中的 200 条/240 秒或 1000 条/1800 秒会迁移为 4000 条/7200 秒。
...
测试使用标准库 `unittest`。前端结构仍由字符串断言覆盖；若本机已安装 Playwright Chromium，还会额外跑 `tests/test_browser_smoke.py` 的无头浏览器冒烟（浏览页加载、公共配置、随机图与瀑布流首批）。CI 默认不强制安装 Playwright。
