...
- **公告与维护模式**：支持 Markdown 公告（含 `![说明](https://...)` 公网图片）、强制阅读倒计时和按公告版本记忆关闭状态；可单独启用联系按钮，支持个人和群组两条联系方式，每条可配图片（二维码等）和跳转链接，弹层一行最多两格、图片在上文字在下，无图片时只显示文字，点击图片或文字跳转对应链接；旧 QQ 号/链接/二维码字段自动兼容为个人联系。维护期间浏览相关 API 返回 503，管理员可在当前会话中使用管理令牌临时解锁。索引重建只在管理页启动，并显示无数据、日期数据、重建中或重建完毕状态。
