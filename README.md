...
一个可通过 GitHub 一键安装的 OpenList 图片 API、终端管理界面和 WebUI。项目只保存通用程序逻辑与默认配置；不包含任何用户特定数据、访问令牌或真实网络地址。

## 特性

- 终端 TUI：安装 OpenList、设置 token 和端口、选择多个图片目录、查看状态、启动/重启服务、重建索引。
- 安全的图片服务：默认仅在本机监听；OpenList API 仅允许配置为本地服务，避免 token 被转发到外部地址。
- WebUI：支持多选 OpenList 目录、单张 / 多张网格 / 瀑布流视图、直接预览 / 下载预览。
...
```bash
curl -fsSL https://raw.githubusercontent.com/Qiscard/OpenList-Image-API/v1.0.0/install.sh | sudo bash
```

安装器会创建图片 API 服务和全局管理命令，但不会自动安装 OpenList。首次运行 TUI 后，选择菜单中的“安装 / 部署 OpenList”即可执行官方命令：
...
- 安装器通过版本标签下载源码，并对运行时 Python 文件和版本文件进行 SHA-256 校验。
...
在创建 `v1.0.0` 标签前，将 `SHA256SUMS` 提交到仓库。后续版本需要同步更新 `VERSION`、`install.sh` 中的 `RELEASE_REF` 和校验文件。
