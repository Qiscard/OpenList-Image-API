...
- **公告与维护模式**：支持 Markdown 公告（含 `![说明](https://...)` 公网图片）、强制阅读倒计时和按公告版本记忆关闭状态；可单独启用 QQ 联系按钮（电脑悬停显示二维码并调起 QQ，手机长按显示二维码并跳转加好友）。维护期间浏览相关 API 返回 503，管理员可在当前会话中使用管理令牌临时解锁。索引重建只在管理页启动，并显示无数据、日期数据、重建中或重建完毕状态。
