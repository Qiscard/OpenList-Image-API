...
- **图片浏览页**：幻灯片视图首批加载 6 张并预加载后续 3 张，保留播放历史，支持自动播放；瀑布流每批加载 20 张，桌面端 3 列、平板 2 列、手机 1 列，新卡进入当前最矮列，按视口优先级出图，滚动到 60% 继续加载。
- **批量换链与预热**：浏览页通过 `POST /api/download-url` 批量解析当前批次的签名直链，并在空闲时预热下一批；同路径请求合并，避免重复请求 OpenList。
- **大图窗口**：点击图片在站内打开预览，支持滚轮/双指缩放、双指捏合、左右旋转、双击复位与拖拽平移；URL 失效时自动刷新，页面切回前台并空闲超时后自动恢复连接。
- **清晰浏览交互**：只有点击“下载”才会通过附件接口下载原文件；视图、间距、自动播放间隔、标签显示与大图目录开关仅保存在当前浏览器，图片文字、目录展示、主题、公告、维护模式、标签与日志由管理员统一配置。
- **标签与投票**：管理员可启用图片点赞/踩与自定义分类标签，支持匿名（IP+UA 哈希）或仅管理员令牌两种投票范围；浏览页顶部标签栏可按分类筛选，内置“垃圾桶”标签用于标记待删除图片并在管理页一键清理。
...
- **网络与并发优化**：HTTP/1.1 Keep-Alive、TCP_NODELAY、gzip、20 路签名链接解析、同路径请求合并，以及可配置容量的签名链接缓存。
- **WebUI**：支持全局目录多选、图片文字、目录展示、主题、网站公告、维护模式、标签与日志配置，目录缓存独立刷新或按目录重建，后台索引重建进度提示，以及不含 token 的配置备份与恢复。
...
图片目录、图片文字、目录展示、主题、网站公告、维护模式、标签与日志属于全局服务器配置，需要 WebUI 管理令牌；视图、间距、自动播放间隔、标签显示与大图目录开关只保存在当前浏览器，不需要令牌。管理令牌就是 TUI 菜单 6 显示的令牌，可直接粘贴到 `/admin` 页面加载服务器配置。
...
- **幻灯片视图**：首批加载 6 张并预加载后续 3 张，保留最近播放历史，可在底部历史条点击回看；支持自动播放间隔（0 表示关闭）。URL 失效时自动刷新，页面切到后台再切回并空闲超过 5 分钟时自动恢复连接。
- **瀑布流视图**：每批加载 20 张并保留图片原始比例；桌面端 3 列，平板与手机自动降为 2 列或 1 列；新卡进入当前最矮列以保持列高接近，按视口位置从上到下、从左到右优先出图，视口外的图片由 `IntersectionObserver` 在接近可视区时再加载，滚动到页面 60% 时继续加载下一批。

点击任意图片会在当前页面打开大图预览，不会跳转或直接下载；大图窗口支持滚轮/双指缩放、双指捏合、左右旋转、双击复位与拖拽平移。只有放大预览中的“下载”按钮会请求附件下载接口。视图、间距、自动播放间隔、标签显示与大图目录开关保存在浏览器 `localStorage` 中，以访问来源为单位隔离，不同设备、不同浏览器互不覆盖；图片文字、目录展示、主题、网站公告、维护模式、标签与日志属于全局配置，由管理员在 `/admin` 统一设置。

开启标签功能后，每张图片下方显示点赞/踩按钮与分类标签，浏览页顶部标签栏可按分类筛选；内置“垃圾桶”标签，标记后可在管理页查看并一键删除对应图片。投票范围可选匿名（按 IP+UA 哈希去重）或仅管理员令牌；同一投票者对同一图片的同类投票只记录一次，重复投票会覆盖旧值。开启网站公告后，访客首次进入会看到 Markdown 公告弹窗；设置强制阅读秒数时，访客需等待指定时间才能“本次关闭”或“不再显示”。开启维护模式后，浏览页仅显示维护提示；管理员在维护面板输入管理令牌验证成功后，当前会话可临时查看与下载图片，访问浏览相关接口时需附带管理令牌。

服务使用多线程 HTTP 服务器并共享 20 个 OpenList 签名解析线程，可供多台设备同时浏览。相同图片的并发签名请求会合并；缓存容量与过期时间可在配置中调整。服务器配置由多名管理员同时保存时采用最后一次保存结果。
...
| `/api/images/random?count=…` | 获取随机图片 JSON，支持 `tag`、`min_likes`、`min_ratio`、`sort` 等标签筛选参数。 |
| `/api/download-url` | POST 批量获取已索引图片的当前签名直链，`fresh=1` 强制刷新。 |
| `/api/status` | 获取图片数量、签名缓存和索引重建状态。 |
| `/api/public-config` | 获取设备浏览默认值、图片文字、目录展示、主题、公告、维护与标签状态。 |
| `/api/tagging/stats?paths=…` | 批量查询图片标签统计。 |
| `/api/tagging/categories` | 获取所有分类标签及其计数。 |
| `/api/tagging/vote` | POST 提交点赞/踩或分类投票。 |
| `/api/admin/config` | 管理令牌校验；GET 读取、PUT 保存全局服务器配置。 |
| `/api/admin/directories?path=…` | 管理令牌；列出目录缓存中的子目录。 |
| `/api/admin/directories/refresh` | 管理令牌；POST 后台刷新目录缓存，`path=` 时只重建该目录。 |
| `/api/admin/rebuild` | 管理令牌；POST 后台重建图片与目录索引。 |
| `/api/admin/backup` | 管理令牌；GET 下载不含 token 的配置备份，POST 上传 ZIP 恢复可编辑配置。 |
| `/api/admin/logs?lines=…` | 管理令牌；读取最近的 journalctl 服务日志。 |
| `/api/admin/tagging/trash` | 管理令牌；GET 查看垃圾桶标签下的图片。 |
| `/api/admin/tagging/trash/delete` | 管理令牌；POST 删除选中的垃圾桶图片。 |
| `/api/admin/tagging/reset?path=…` | 管理令牌；POST 重置指定图片或全部标签数据。 |
| `/health` | 健康检查。 |
...
- WebUI 的服务器接口只允许修改目录、图片文字、目录展示、主题、网站公告、维护模式、标签与日志；设备浏览偏好仅写入当前浏览器，不能通过 WebUI 修改 OpenList API 地址、token 文件或图片扩展名。
