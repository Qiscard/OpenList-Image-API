...
- **索引与浏览性能**：重建是后台一次性任务（约 300 个作者相册时，本机 OpenList 通常约 1.5–5 分钟；对接百度一刻上游的生产机可能需要更久，根目录列举单页就常超过 10 秒），浏览随机图/瀑布流只读本地 `index.json` 再按需换链，不因作者目录数量在每次请求时重扫存储。索引列表超时 60 秒、每页 100 条，避免一刻根目录被过短超时打成 0 图。
