...
- **图片浏览页**：单张视图按批缓存 5 张；网格与瀑布流每批加载 15 张。桌面网格固定 3 个位置，宽图自动跨 2 列；瀑布流按固定列从底部稳定追加，不再重排旧图片。
- **清晰浏览交互**：点击图片打开站内放大预览，只有点击“下载”才通过附件接口下载原文件。视图与间距保存在当前浏览器，图片文字由管理员统一配置。
- **网络与并发优化**：HTTP/1.1 Keep-Alive、TCP_NODELAY、gzip、12 路签名链接解析、同路径请求合并，以及默认 1000 条/30 分钟的签名链接缓存。
- **WebUI**：支持全局目录多选、扩展名与图片文字设置、后台索引重建进度提示，以及不含 token 的配置备份。
...
图片目录、扩展名和图片文字属于全局服务器配置，需要 WebUI 管理令牌；视图与图片间距只保存在当前浏览器，不需要令牌。管理令牌就是 TUI 菜单 6 显示的令牌，可直接粘贴到 `/admin` 页面加载服务器配置。
...
- **网格视图**：每批加载 15 张；桌面端固定每行 3 个位置，宽高比不低于 1.45 的宽图自动横跨 2 个位置，平板与手机自动降为 2 列或 1 列。
- **瀑布流视图**：每批加载 15 张并保留图片原始比例；使用独立固定列轮转追加，新批次只进入各列底部，不会像 CSS 多栏布局那样重排旧图片。

点击任意图片会在当前页面打开大图预览，不会跳转或直接下载；只有图片卡片或放大预览中的“下载”按钮会请求附件下载接口。视图与间距保存在浏览器 `localStorage` 中，以访问来源为单位隔离。不同设备、不同浏览器互不覆盖；图片文字属于全局配置，管理员可选择完整路径、仅图片名称或不展示。
...
| `/api/public-config` | 获取设备浏览默认值和全局图片文字设置。 |
...
- WebUI 的服务器接口只允许修改目录、扩展名和图片文字；设备浏览偏好仅写入当前浏览器，不能通过 WebUI 修改 OpenList API 地址或 token 文件。
