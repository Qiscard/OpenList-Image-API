...
服务端共享 6 个 URL 解析线程；OpenList `fs/get` 换链超时 45 秒。批量接口最多接收 50 个有效去重路径，并在 25 秒后为未完成项返回 `url resolve timed out`。超时后上游解析仍继续填缓存，不取消。未索引路径和上游失败按路径返回独立错误。一刻相册单图 `fs/get` 在生产环境常见 15–30 秒，过短客户端超时会把 OpenList 上游请求 cancel，表现为图库空白与大量 502。
