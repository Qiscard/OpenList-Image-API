...
│   └── test_management_features.py # 管理、TUI、安装器相关测试
├── .gitattributes
├── .gitignore
├── LICENSE                        # MIT
├── README.md                      # 用户文档
...
文件体量集中在 [src/openlist_image_api.py](file:///e:/Other/Github/OpenList-Image-API/src/openlist_image_api.py)（约 1830 行，含内嵌前端代码）。仓库不包含 `requirements.txt`、`pyproject.toml`、`Dockerfile` 或 `docker-compose.yml`。
...
| OpenList 客户端 | `OpenListClient` | 分页列目录、解析文件签名直链、3 次重试 |
...
| 应用核心 | `Application` | 配置持有、状态聚合、管理配置更新、备份恢复、后台刷新 |
...
使用标准库 `unittest`，向 `sys.path` 注入 `src` 后导入被测模块。`test_core.py` 覆盖配置、缓存并发、索引、状态、备份；`test_management_features.py` 覆盖管理配置、公告版本、WebUI 结构、下载响应、TUI 与安装器约束（含禁止 Docker 管理）。
...
| GET | `/api/status` | — | 聚合状态 |
| GET | `/api/public-config` | — | 访客可见配置 |
| GET | `/api/images/random` | 维护模式 | 随机图片 JSON（含签名 URL） |
| GET | `/api/download-url?path=…&fresh=…` | 维护模式 | 单图签名直链，`fresh=1` 强制刷新 |
...
| POST | `/api/admin/directories/refresh` | 管理令牌 | 启动后台目录缓存刷新 |
...
浏览器 GET /api/images/random?count=15
  ...
  → Application.resolve_images
      ...
      → 写入 LRU 缓存
  → 12 线程 ThreadPoolExecutor.map 并发
  → 返回 [{path, size, url}, ...]
...
- **布局**：幻灯片（`single`）与瀑布流（`waterfall`），桌面端瀑布流固定 3 列，宽高比 ≥1.45 的图片跨 2 列，平板/手机降为 2/1 列；按固定列轮转追加，避免重排旧图。
- **幻灯片**：每批预加载后续 3 张，保留最近播放历史（上限 60），支持自动播放间隔（0 关闭），页面隐藏/灯箱打开/设置面板打开时暂停。
...
- **偏好隔离**：视图、间距、自动播放间隔、大图目录开关存 `localStorage`，按访问来源隔离；图片文字、目录展示、公告、维护模式为全局服务器配置。
...
- 目录缓存刷新：`POST /api/admin/directories/refresh`，随后轮询 `/api/status`。
...
| `url_cache_size` | `1000` | 0–5000 |
| `url_cache_ttl_seconds` | `1800` | 0–3600 |
| `admin_token_file` | `/etc/openlist-image-api/admin.token` | 绝对路径 |

### 8.3 WebUI 可编辑字段

仅以下字段可通过 `PUT /api/admin/config` 修改：`directories`、`caption_mode`、`directory_display_enabled`、`directory_display_depth`、`announcement_*`、`maintenance_enabled`（[openlist_image_api.py:481-504](file:///e:/Other/Github/OpenList-Image-API/src/openlist_image_api.py#L481-L504)）。`openlist_api_url`、token 文件路径、扩展名等受保护字段不能通过 WebUI 修改。
...
- `migrate_performance_defaults`：`grid_scale` 125→150、`url_cache_size` 200→1000、`url_cache_ttl_seconds` 240→1800，仅迁移仍为旧默认值的项（[install.sh:114-137](file:///e:/Other/Github/OpenList-Image-API/install.sh#L114-L137)）。
...
无第三方依赖。导入均为标准库：`argparse`、`gzip`、`hmac`、`io`、`json`、`logging`、`os`、`random`、`secrets`、`socket`、`threading`、`time`、`zipfile`、`collections`、`concurrent.futures`、`http`、`pathlib`、`typing`、`urllib`（[openlist_image_api.py:1-27](file:///e:/Other/Github/OpenList-Image-API/src/openlist_image_api.py#L1-L27)）。TUI 额外使用 `getpass`、`shutil`、`subprocess`、`sys`、`urllib.request`。
